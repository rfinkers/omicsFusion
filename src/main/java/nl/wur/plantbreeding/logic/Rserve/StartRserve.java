...
class StreamHog extends Thread {
    ...
    /** The logger */
    private static final Logger LOG = Logger.getLogger(StreamHog.class.getName());
 ...
 * mainly <code>checkLocalRserve</code> method. It spits out quite some debugging
 * outout of the console, so feel free to modify it for your application if desired.<p>
<i>Important:</i> All applications should shutdown every Rserve that they started!
 * Never leave Rserve running if you started it after your application quits since
 * it may pose a security risk. Inform the user if you started an Rserve instance.
 */
public class StartRserve {
     ...
     * @param debug <code>True</code> to enable debug output
     * @return <code>true</code> if Rserve is running or was successfully started, <code>false</code> otherwise.
     */
     ...
     * @return <code>true</code> if server is running or sucsessfully started. <code>False</code> if there was an error.
     */
    public static boolean checkLocalRserve() {
        ...
        if (osname != null && osname.length() >= 7 && osname.substring(0, 7).equals("Windows")) {
            ...
            try {
                Process rp = Runtime.getRuntime().exec("reg query HKLM\\Software\\R-core\\R /s"); //someone deleted the /s but it is necessary!! Roeland 2010-04-13
            ...
            if (installPath == null) {
                System.out.println("ERROR: cannot find path to R. Make sure reg is available and R was installed with registry settings.");
                return false;
            }
